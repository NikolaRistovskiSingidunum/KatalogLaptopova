...
-- Records of display
-- ----------------------------
INSERT INTO `display` VALUES (1, 19.0, '800x600', 0);
INSERT INTO `display` VALUES (2, 25.0, '1024x960', 1);
INSERT INTO `display` VALUES (3, 32.0, '1920x1080', 0);
...
-- Records of gpu
-- ----------------------------
INSERT INTO `gpu` VALUES (1, 'integrated', 'RADEON', 6);
...
-- Records of laptop
-- ----------------------------
INSERT INTO `laptop` VALUES (1, 'HP 544', 36000.00, '1.jpg', 'WINDOWS 10', 'US', 0, 0, 3, 1, 1, 1, 4, 'DDR3', 'HP', '2019-04-25 22:15:06');
INSERT INTO `laptop` VALUES (2, 'Huawei P4', 45000.00, '2.jpg', 'windows 8', 'UK', 1, 0, 2, 1, 2, 2, 16, 'DDR4', 'HUAWEI', '2019-04-25 22:23:42');
INSERT INTO `laptop` VALUES (31, 'Aorus AI', 700000.00, '31.jpg', 'WINDOWS', 'US', 0, 0, 4, 2, 2, 2, 12, 'DDR4', 'AROUS', '2019-06-14 17:16:27');
INSERT INTO `laptop` VALUES (36, 'Asus E104', 24000.00, '36.jpg', 'WINDOWS', 'US', 0, 0, 1, 1, 1, 1, 4, 'DDR3', 'IBM', '2019-06-16 14:23:22');
INSERT INTO `laptop` VALUES (37, 'Apple A4', 800000.00, '37.jpg', 'IOS', 'US', 1, 0, 3, 3, 3, 2, 8, 'DDR4', 'Apple', '2019-06-24 22:26:00');
...
CREATE TABLE `purchase`  (
  ...
  INDEX `fk_purchase_laptop_id`(`laptop_id`) USING BTREE,
  CONSTRAINT `fk_purchase_laptop_id` FOREIGN KEY (`laptop_id`) REFERENCES `laptop` (`laptop_id`) ON DELETE RESTRICT ON UPDATE CASCADE
...
-- ----------------------------
DROP TRIGGER IF EXISTS `trigger_purchase_bi`;
delimiter ;;
CREATE TRIGGER `trigger_purchase_bi` BEFORE INSERT ON `purchase` FOR EACH ROW BEGIN
	IF NEW.surname NOT RLIKE '^[A-Z][a-z]{1,63}$' THEN
		SIGNAL SQLSTATE '50020' SET MESSAGE_TEXT = 'Prezime je pogresnog formata!';
	END IF;
	
	IF NEW.forename NOT RLIKE '^[A-Z][a-z]{1,63}$' THEN
		SIGNAL SQLSTATE '50021' SET MESSAGE_TEXT = 'Ime je pogresnog formata!';
	END IF;
		
	IF NEW.phone NOT RLIKE '^[0-9]{7,63}$' THEN
		SIGNAL SQLSTATE '50022' SET MESSAGE_TEXT = 'Broj telefona je pogresnog formata!';
	END IF;
END
;;
delimiter ;
...
-- Triggers structure for table purchase
-- ----------------------------
DROP TRIGGER IF EXISTS `trigger_purchase_bu`;
delimiter ;;
CREATE TRIGGER `trigger_purchase_bu` BEFORE UPDATE ON `purchase` FOR EACH ROW BEGIN
	IF NEW.surname NOT RLIKE '^[A-Z][a-z]{1,63}$' THEN
		SIGNAL SQLSTATE '50020' SET MESSAGE_TEXT = 'Prezime je pogresnog formata!';
	END IF;
